...
import { cosineSimilarity } from "../../utils/response.js";
  ...
  async behaviorRecall(ctx: RecallContext): Promise<AttractionCandidate[]> {
    ...
    const [historyRows] = await this.pool.execute(
      `SELECT target_id AS attractionId, target_type
       FROM (
         SELECT target_id, target_type, created_at FROM favorites
           WHERE user_id = ? AND target_type = 'attraction'
         UNION ALL
         SELECT attraction_id AS target_id, 'attraction' AS target_type, created_at FROM reviews
           WHERE user_id = ?
         UNION ALL
  ...
  async collaborativeRecall(ctx: RecallContext): Promise<AttractionCandidate[]> {
    ...
    const [similarUsers] = await this.pool.execute(
      `SELECT other.user_id, COUNT(*) AS overlap
       FROM (
         SELECT user_id, target_id FROM favorites WHERE target_type = 'attraction'
         UNION
         SELECT user_id, attraction_id AS target_id FROM reviews WHERE rating >= 4
       ) other
       INNER JOIN (
         SELECT target_id FROM favorites WHERE user_id = ? AND target_type = 'attraction'
         UNION
         SELECT attraction_id AS target_id FROM reviews WHERE user_id = ? AND rating >= 4
    ...
    const [rows] = await this.pool.execute(
      `SELECT
       ...
       FROM (
         SELECT user_id, target_id AS attraction_id FROM favorites WHERE target_type = 'attraction'
         UNION ALL
         SELECT user_id, attraction_id FROM reviews WHERE rating >= 4
       ) acts
